...
### 2026-09-23: ⑬の選択式ワープは停止式ではなく通過式
- ユーザー再確認: 初戦・再戦共通の`kessan`のXは、ぴったり停止不要。踏んだ時点で対岸の3候補から選択して転移し、残り歩数を続行する。出口への到着そのものでは再転移しない。
- `warpOnPass`を設定。次ダイス2倍は⑧のワームホールだけに限定（⑬へ流用しない）。通過経路と対人配信用区間は別々に保ち、歩行→選択→転移→残り歩行の順序を守る。
- 着地予測とCPU距離探索は`_passWarpDestinationIds`で全出口を辿る。先頭のCP島だけに固定しない。予測の同一歩数・移動状態を集約して探索の膨張を防ぐ。
- セーブ復元はワープ接続等の固定マップ情報を上書きしない。選択中の退出後に古い移動を再開しないガードも追加。
- 回帰: `tests/choiceWarp.test.mjs`（test:story登録）。6入口の通過/停止、残り歩数、二重転移なし、予測/CPU、選択中退出、旧セーブ、⑦/⑧互換。キャッシュv332（過去に使用した番号を再利用しない）。
- test:all・lint・build通過。ローカル実UIを844×390のタッチ環境で起動し、4人同盟・手札ありで出目5→中央の分岐→CP取引を閉じる→4歩目でワープ選択→出口の分岐で残り1歩→土地メニュー終了まで確認。iPhone実機は未確認。

### 2026-09-21: 戦闘装備のHP/ATK表示を実計算へ統一
- 実ダメージ計算は変更しない。装備公開がカードの素のHP/ATK加算を表示していたため、Ninjaの2倍・くぐつの剣豪のHP増加不可と食い違っていた。
- `equipmentStatDelta` は能力交換後のユニットに `statTotals` を使い、既に公開した装備との差分を返す。通常装備・強奪した装備の3経路から `onBattleEquip` に `appliedAtkBonus/appliedHpBonus` を渡す。既存の対人イベント配信でそのまま送る（新フックなし）。古いイベントは元の数値へフォールバック。
- 貫通で除くのは引き続き土地レベルHPのみ。表示上はHPマイナス装備と土地HPが相殺しても土地分を全量差し引き、負数チップも残す。装備HPやカード効果HPまで取り消さない。
- `tests/battleEquipmentDisplay.test.mjs` をtest:storyへ追加。実UI関数と計算の突合、強奪累計、ソケット交換、貫通＋HP減少を8件で確認。test:allは238件通過。キャッシュv331。
- ユーザー報告の具体的なモンスター＋装備の組合せはまだ未回答。怨念の貫通で通常装備HPそのものが消える現象は再現できず、確認済みの表示不整合を修正した。

### 2026-09-20: ⑲クエの周回札を1枚追加
- `queFinal` のキャンセルカルチャー2→1、アイキャンフライ2→3。合計40枚、チヌ・他ステージのクエ・AIは変更なし。対応する固定デッキ回帰テストを更新。キャッシュv330。

### 2026-09-20: ⑲サーティーの救援デッキ再調整
- `thirtyFinal` だけ変更（⑪などの `thirty` は維持）。言論封殺1・不死鳥の呪い1・フリーランサー2・世界樹2を外し、千本桜2・くぐつの剣豪2・イカサマのサイコロ2へ。モンスター18／アイテム12／スペル10の40枚。
- ブリモン1枚は主人公の当該盤面デッキの定義を独立コピーする。主人公のデッキに無い時は選択中のブリード設定を使用。能力・属性・コスト・画像を継承し、参照共有による相互の成長／戦闘状態の混入を避ける。一覧など主人公情報がない場合のみ初期形を表示。
- `startStoryBattle` の救援設定に `allyDef.aiProfile` を渡す（設定済みの救援用AIが実戦に届いていなかった）。シミュレーターも `allyA=thirtyFinal` に主人公のブリモンを渡す。
- 回帰テストで40枚・指定札枚数・独立コピー・⑪不変を確認。`npm run test:all`、lint、build通過。キャッシュv329。
...
2026-09-20追記: ユーザー提供`ending.mp3`を`public/audio/ending.mp3`へ無加工で収録済み
（約120.050秒）。`PENDING_TRACK_FILES`から解除し、サウンドルームへ
「♪エンディング」として追加。以下の「音源未着」は実装時点の履歴。
専用カットシーン再生処理は維持。Windows改行で関数の切り出しが失敗する
回帰テストをLF正規化して修正（実際の再生不具合ではない）。
...
バンドルもパス書き換えもされない。
- そのため**リンクは必ず相対パス**（`../audio/…`, `../images/stage/thumb/…`）。
  GitHub Pagesでは`base`が`/chinu-quest2/`になるので、絶対パスだと即死ぬ。
- 音源は`public/audio/`の実ファイルをそのまま再生する（複製していない）。
- 一覧のサムネは`public/images/stage/thumb/sNN.jpg`（背景を幅720pxへ縮小、
  計約900KB）。元の背景は1枚2MB前後あり、17枚並べると重すぎるため。
  ⚠️ **背景を差し替えたらサムネも作り直すこと**（`ffmpeg -i <元> -vf scale=720:-2
  -q:v 5 public/images/stage/thumb/sNN.jpg`）。
...
- デッキ内のクエ由来カード: 川田40枚のうちクエのデッキと被るのは
  ...
  入る汎用札）と**資本主義の権化**。EXでクエ固有なのは資本主義の権化だけだが、
  **この札はセリフを喋らない**ので「クエが喋る」という報告の原因ではない。
  なお採用の是非は2026-09に「いまのままでいい」＝採用のままで決着している
  （下の「資本主義の権化は採用のまま確定」節）。
...
## 🚫 不可侵のルール: 呪いは重なったら「後から撃たれたものが有効」

> 「モンスター呪いもプレイヤー呪いも重なったらあとから撃たれたものが有効だから。
> これは不可侵のルールね」（2026-09、ユーザー指定）

**加算・重ねがけ・累積は一切作らないこと。** 新しい呪いを足す時も、既存の呪いを
触る時も、必ず「上書き」で実装する。
...
⚠️ **以前は`tile.transparentCursed`という土地フラグだった**が、
「あれはモンスターへの呪いなので、他の呪いで上書きできる」というユーザー指定で
**モンスターの呪い**へ作り替えた。効果は**侵略不可・通行料0・単体スペルの
対象にならない**の3つ。

- 実体は`applyCurse(unit, { name:'聖域', traits:['sanctuary','singleTargetImmune'] })`。
  `unit.curses`は**1つしか保持しない**（battle.jsの`setCurse`が置き換える）ので、
  **後から別の呪いをかければ聖域は自動的に消える**。これがユーザーの言う
  「他の呪いで上書きできる」。
- モンスターが死ぬ・移動する・入れ替わる時も個体ごと消えるので、
  **土地側のフラグを片付ける処理が要らない**。以前は`transparentCursed = false`を
  30か所以上で消して回っており、1か所でも漏れると「誰も居ない土地が永久に
  侵略不能」になる作りだった。
- 判定は必ず**`_isTransparentTile(tile)`**を通す。土地フラグ（透過の呪い＝
  `curseTransparency`。モンスターの土地コマンドで土地そのものに付く別物）と、
  聖域の呪いの両方を見る。⚠️ `tile.transparentCursed`を直接読まないこと。

### 干渉できるのは全体スペルだけ（ユーザー指定 2026-09「全体スペルだけでいいんだよ」）
- 聖域の呪いは既存の**`singleTargetImmune`**特性をそのまま付ける。判定は
  **`_isSingleTargetImmuneUnit(unit)`**の1本で、**スペルも土地コマンドの
  特殊効果（火炎瓶男の射程ダメージ・センチネルの自爆ダメージ等）もまとめて
  弾く**。使用箇所はスペルの対象選択・`_applySpellEffect`の最終ガード・
  CPUの各スペルAI・`_cpuCastSpell`・能力の対象収集（人間/CPU両方）。
- ⚠️ **聖域の土地は侵略もできない**ので、結果として**干渉手段は全体スペルだけ**
  になる。90Gで自分の通行料を捨てる代償はあるが、防御ロックとしては非常に強い。
  **これは意図した強さ**（一度「能力は通す」形で実装したが、ユーザー判断で
  能力も塞いだ）。弱めるならまずここを分割する。
- **全体スペルは普通に通る**。パンデミック・小隕石・洪水・土地神の怒り等は
  `target:'none'`で対象選択を経由しないので、上記のガードに一切かからない。
  聖域を剥がす現実的な手段もこれ（パンデミックなら個体ごと置き換わる）。
- ⚠️ `_unitHasTrait`（game.js）は**def/awakenedしか見ない**。battle.jsの
  `hasTrait`はitems/cursesも見るが、game.js側の同名ヘルパは見ないので、
  **呪い由来の特性はそれぞれの判定関数で別途拾う必要がある**。ここを
  勘違いすると「呪いを付けたのに効かない」になる。
- ⚠️ 副作用: 「関所つぶし」AI(`sanctuaryCounterForcedStop`)は**敵**の
  モンスターへ聖域を撃って通行料をゼロにするが、その敵モンスターにも
  スペル耐性を与えることになる。狙いは通行料の無効化なので許容している。
- 回帰テスト: `npm run test:cards`「聖域はモンスターへの呪いとして付く」
  「聖域は別の呪いで上書きされて消える」「聖域はモンスターが居なくなれば
  自動的に消える」「聖域中のモンスターは単体スペルの対象一覧に出ないが、
  全体スペルは通る」「聖域中のモンスターは土地コマンドの特殊効果も
  受け付けない（残るは全体スペルだけ）」。旧実装（土地フラグ）へ戻すと4件、
  `_isSingleTargetImmuneUnit`が呪いを拾わなくなると3件が落ちることを確認済み。
...
⚠️ **上記はCodexの`_cpuSummonReserve`変更(b64b1e6)より前の数字。** 同変更
（召喚後に最低300Gを土地レベルアップ用に残す・全CPU共通）の後、同じ条件・
同じseed(21-24)で測り直すと**効果はむしろ倍以上に伸びた**:

| 構成 | 敵側勝率（seed21-24・各120戦） |
|---|---|
| fixer無効 | 24/120 **20.0%** |
| **fixer有効（現行）** | 50/120 **41.7%** |

差**+21.7pt**、SE 5.8pt＝**3.7σ**。⑰の敵側は現在**41.7%**。
🔗 伸びた理由の見立て: Codexの変更でCPUが土地レベルアップ用のGを残すように
なり、**お札の価格は自陣の色付き土地のレベル合計で決まる**ので、fixerが
仕込んだお札の含み益がより伸びるようになった（お札と土地育成が噛み合った）。
⚠️ **全CPU共通の変更はお札マップの勝率を動かす**ので、この手の変更が入ったら
⑫⑬⑮⑰は測り直すこと。
...
- `format:'2vs2'`、`heroAllianceId:'red'`/`enemyAllianceId:'white'`、
  味方`ally`＝川田（⑮の`kawada`をそのまま流用。ユーザー指定）。
- 敵は`roudouMuuru`（ムール）と`roudouDanball`（ダンボール男）。
- ⚠️ **マダイは冒頭の会話に出るだけで対戦には絡まない**。opponents/allyに入れない。
- ⚠️ outroの川田の台詞は`主人公`を**丸括弧なしで素書き**する。`withHeroName`
  （main.js）が表示直前にプレイヤーのキャラ名へ置換する。`（主人公）`と
  書くと置換にマッチせず「主人公」のまま出る。
- 回帰テスト: `npm run test:cards`「⑰海底労働施設のステージ定義（仮実装）」
  「⑰の盤面は洞窟型39マス・CP3・4属性均等（連鎖は全属性3+3+1+1）」。
  後者は連結性・行き止まり2つ・属性の連鎖分布まで実際に辿って検証する。
...
- ⑰の仕様は**全て確定した**（盤面・会話・目標総資産・敵味方のデッキ）。
  残るのは実装そのものと、川田を⑮の`kawada`のまま出すかどうかの最終確認。

### 敵デッキ: ダンボール男（`roudouDanball` — 確定）
⚠️ **⑤の`danball`とは別キー**（⑤「暗転した世界」の難度を動かさないため）。

⑤の`danball`（無11・火6・雷6）からの差し替え（2026-09、ユーザー指定）:
...
結果は40枚（モンスター23／アイテム10／スペル7）、属性は**無11・雷7・火4・森1**。
- ⚠️ **テンホウが3枚なのは枚数合わせ**。「カエンタケ・人魂（計3枚）→門番兵1・
  魔導士1（計2枚）」で1枠余ったので、ユーザー判断でテンホウを2→3枚にした。
- **古代のギア9枚（ガシャーン合体の素）はそのまま**。洞窟マップは分岐が多く
  足が止まりやすいので、10Gを撒く戦法はむしろ相性が良い。
- ⚠️ **くぐつの剣豪は特殊マスを通れない**。⑰の盤面はCP3つが要所にあるので、
  分断された側へは渡れない。⑯で中央CPをくぐつ対策に置いたのと同じ性質。
- 🔗 火が6→4に減って雷が6→7に増えた。テンホウ3枚の「与ダメージ×3G強奪」で
  相手の手持ちGを削る形が主軸になる。
- 回帰テスト: `npm run test:cards`「⑰のダンボール男専用デッキ(roudouDanball)は
  40枚で、⑤のdanballを壊していない」。差し替えを1枚単位で固定し、ギア9枚と
  ⑤側の属性構成が動いていないことも見る。

### 敵デッキ: ムール（`roudouMuuru` — 確定）
⚠️ **⑨の`muuru`とは別キー**にしてある。直接いじると⑨「暴君と税務調査」の
難度が動くため。⑯が`chinuMuuru`を別キーにしたのと同じ扱い。

⑨の`muuru`（水19・無1）からの差し替え（2026-09、ユーザー指定）:

| 抜いた | 入れた |
|---|---|
| アオリイカ×2 | ボムボックリ×2（森40G 1/1・死亡でボックリ2体を空き地へ） |
| 氷柱×2 | アイランドホエール×2（成長型の水） |
| オサフネ×1 | イカサマのサイコロ×1 |
| 薄氷の剣×1・ナンカのお守り×1 | 水神の盾×2 |
| 追徴課税×1・不動産鑑〇士×1・タフネス×1 | 避雷針侍×2・バックファイア×1 |
| ブルーオーシャン×1・副業収入×1 | サイコキネシス×1・帰巣本能×1（1→2枚） |

結果は40枚（モンスター22／アイテム8／スペル10）、属性は**水17・森2・雷2・無1**。
- ⚠️ **成長型の水属性はアイランドホエール1種しか無い**（他の成長型は
  溶岩竜=火／甲鉄要塞=雷／巨珍兵=森／積み上がった伝票=無）。「成長型の水」を
  指定された時の選択肢はこれだけ。⑮の川田も3枚積んでいるので、⑰では
  両陣営が同じ壁を並べる形になる。
- ⚠️ アイランドホエールは**空き地への通常召喚専用**で移動・侵略に使えない。
  数えるのは壁としてだけ。
- 🔗 **役割が⑨から変わっている**。追徴課税・不動産鑑〇士・ブルーオーシャン・
  副業収入（通行料と金回りを太らせる調査官の札）が全部抜け、避雷針侍・
  バックファイア・サイコキネシス・ボムボックリという「相手を進ませない／
  倒されても土地を増やす」看守寄りの構成になった。⑰の舞台に合う。
- ⚠️ **味方の川田も水軸で水神の盾を持つ**ので、⑰は盾同士のにらみ合いになる。
  盾同士の挙動はCLAUDE.mdの属性神の盾の節を参照（2026-09に貫通は撤去済み）。
- ⚠️ 盤面の水マスは8マスしかない。ムール（水17）と川田（水15）で取り合う。
- ⚠️ 避雷針侍は**召喚に手札1枚の生贄**が要る。手札が細ると出せない。
- 回帰テスト: `npm run test:cards`「⑰のムール専用デッキ(roudouMuuru)は40枚で、
  ⑨のmuuruを壊していない」。差し替え内容を1枚単位で固定し、⑨側の水単構成
  （水19・無1）が動いていないことも同時に見る。

## ⑱「チヌ単体」／⑲「チヌ＋クエ」構想（2026-08 ユーザー指定）
⑰でムールとダンボール男を倒した後の決戦。
**⑱は実装済み**（2026-09、下の「⑱の盤面とデッキ」「⑱の数値調整」参照）。
...
2026-09-19更新: 救援条件はユーザー再指定により**3,000G差のまま維持**（12,000G案は不採用）。
救援会話は「クエを追ってきたら…」から「チャンスだ！」まで指定4行、続けて
チヌ「...フン、おまえもその程度だったか。」の計5行で盤面へ戻る。
`midBattleAssist.overlay`で主人公を左・サーティーを右に表示し、最終行だけ左をチヌに差し替える。⑪の闇ホフクを流用しない。
サーティーの会話立ち絵だけ約15%拡大（⑪・⑲、通常会話にも適用。低い画面は上端切れ防止の上限あり）。
⑱にサーティーの出番はないため会話・登場人物は追加しない。駒サイズ・救援の進行処理は変更なし。

**設定（ユーザー指定）: クエはチヌの「表の姿」が表面化したもの＝金融街のフィクサー。**
チヌ（怨念の塊＝裏）とクエ（相場を握る表）は元から一人。introでチヌが明かし、主人公が
「表の顔が既にフィクサーかよ！！」とツッコむ。クレジットの「企画・クエ（チヌ）」も同じ。
⑫⑬でクエが名乗った「ハタ類の王」「この世はワイがルールや」はこの伏線として読める。
...
- **サーティー参戦**: `midBattleAssist.enemyAssetsLeadAtLeast: 3000`。敵陣営の合算総資産が
  ...
  絶対差の分岐を追加）。
  ⚠️ **発火が早すぎる懸念**: 敵は2人で開始時から合算2,000G vs 1,000G。土地を取り
  始めれば数手番で差3,000に届くので、実質ほぼ毎回サーティーが来る可能性が高い。
  2026-09-20の救援ありCPU比較では現行版も連携版も独立48戦中47戦で参戦した。
  **ユーザー指定で条件は維持**。難度調整のために無断で救援を遅らせない。
- **結末の分岐**（main.js `handleStoryBattleEnd`）: `assisted`（game側の
  `storyAssistTriggered`＋参戦後セーブからの再開用にサーティーの在席）で分岐。
  - 参戦あり＋勝利 → **バッドエンド** `stage.assistOutro`（主人公がチヌの瘴気で闇堕ち→
    独裁者。真エンドを匂わせる）。クリア扱いだが報酬なし。
  - 参戦なし＋勝利 → **真エンド** `stage.outro`＋`trueEndReward:'onnenNoShuugoutai'`
...
- **真エンドは「激ムズでいい」（ユーザー指定）。** 人間の攻め筋はパンデミックなので、
  ⑲のチヌは決戦版デッキ`chinuFinal`（2026-09-20のコンビ連携版は下記）。
  **言論封殺2・キャンセルカルチャー4**を維持し、怨念の集合体4・フリーランサー2・
  ぺーの杖4で侵略と周回経済を両立。⑱のデッキとは別。合計40枚。
...
- **過去の計測（連携強化前。目標24,000・両者1,000G・seed21/41×20戦、主人公役はひなんじょデッキ）**:

| チヌの狩り | 真エンド経路（1vs2） | バッドエンド経路（サーティーが最初から居る2vs2で近似） |
|---|---|---|
| huntMinLandLevel 2（⑱と同じ） | 主人公 2/40 = **5%**（総資産8k止まり） | 主人公側 6/40 = **15%**（サーティーは土地2〜6枚で狩られる） |
| **huntMinLandLevel 3（⑲仮採用）** | 主人公 3/20 = **15%**（seed21） | 主人公側 10/20 = **50%**（seed21） |

  当時はLv3を仮採用。2026-09-20のユーザー依頼「コンビのデッキとAIを強化」で
  Lv2に変更し、下記の新デッキで再検証した。目標24,000G・救援の差3,000Gは維持。

### ⑲クエの経済デッキ（2026-09-19時点の検証記録。現行は次節）
...
### ⑲チヌ＆クエのコンビ連携強化（2026-09-20、現行）
- 対象は⑲の`chinuFinal` / `queFinal`とステージ固有AI設定だけ。
  ⑫⑬⑱のデッキ、王型46マス・お札、目標24,000G、各人1,000G、救援3,000G差、
  救援会話・結末・報酬・戦闘計算・対人通信は変更しない。`game.js`変更なし。
- **チヌ40枚（21/12/7）**:
  - テンホウ4、サンダーバード4、Ninja4、避雷針侍2、めたんまん1、フリーランサー2、怨念の集合体4。
  - ぺーの杖4、イカサマのダイス3、ナンカのお守り3、真剣白刃取り2。
  - 言論封殺2、キャンセルカルチャー4、資本主義の権化1。
- **クエ40枚（16/6/18）**:
  - 甲鉄要塞4、鉄男4、サンダーバード4、フリーランサー2、テンホウ2。
  - ナンカのお守り4、ライフジャケット2。
  - アイキャンフライ4、帰巣本能4、副業収入4、資本主義の権化2、言論封殺1、放電1、ホライズン2。
    （同日追記: ユーザー指定で資本主義を2枚減らし、封殺とホライズンを1枚ずつ追加。下記参照。）
- チヌは`huntMinLandLevel:2`で怨念の侵略対象を早期から取る。既存の勝算・資金判定は維持。
  `levelPumpSignal:{allyName:'クエ',elements:['thunder'],toLevel2:0,unleash:20}`で、
  クエの雷お札が20枚未満なら土地投資はLv2まで、20枚以上なら通常上限へ解放する。
  仕込み前もLv2にできるため、クエが購入できない時に投資が完全停止しない。
  解放後は払える最大段階（1回+3段階まで）。手持ち300G以上・投資後現金250G・通行料余力は免除しない。
- クエは従来の`fixer/lapRacer/侵略勝算0.9/高額地回避0.9/雷お札連携`を維持し、
  `scatterSummons:true`追加。無属性空地は低コスト優先、有属性地は従来の属性・強さ判定。
  召喚予備費の基準は120G（危険な通行料は別途確保）、通常価格帯のお札投資予備費は450G。
  お札が下限近辺なら既存の底値買いロジックを使う。フリーランサー効果は重複しない。
- **初回強化時の比較条件**（クエは資本主義4・ホライズン1・言論封殺なしだった時点）:
  基準はコミット`3416636`。主人公役は`@tools/decks/hinanjo-deck2.json`をCPU操作。
  `tools/simulate.mjs`の本体をメモリ内で読み込み、`Game.init`に⑲の実際の
  `midBattleAssist`を設定。救援判定の間だけ主人公のisCPUをfalseにし、実装済みの
  条件・サーティー追加処理を通す。操作・描画はシミュレータ代役で、本番エンジンの数値計算を使用。
  `--allyA=none --map=ou --goal=24000 --start=1000 --maxTurns=600 --timeout=60000`。
  比較中は基準デッキとプロフィールを固定し、編集後の定義へ差し替わらないようにした。
- 予備比較（seed1101、各24戦、敵チーム勝数）: 基準15、装備強化16、投資連携19、
  防御重視15、早期侵略20、**早期侵略＋投資連携21**。最後の組合せを採用。
- 独立シードの追加比較（seed1701、各48戦）:

  | 構成 | チヌ＋クエの勝数 | 平均決着手番 |
  |---|---|---|
  | 基準3416636 | 41/48（85.4%） | 181.3 |
  | 投資連携のみ・狩りLv3 | 33/48（68.8%） | 189.9 |
  | 採用版・狩りLv2＋投資連携 | 41/48（85.4%） | 172.1 |

  signatureは順に`f54e24de53eadac8` / `a0b3eb78eb6997b7` / `aeb0590dcba3c821`。
  採用版は最終敵チーム資産平均30,041G（基準26,605G）。**追加比較では勝率は同じ**で、
  予備比較の上昇を一般化しない。人間の実勝率・全デッキへの最強保証ではない。
- 実装後の実デッキキー・プロフィールでseed1101の先頭8戦を再実行し、候補と全勝敗・
  手番数が一致（7/8勝、`654cf13ab3d82e13`）。パンデミック入り`fusagikonda`相手は
  seed2301で8/8勝（`7c4d40a56c782edf`）。少数の動作確認であり無敵の証明ではない。
  予備144戦＋追加144戦＋再確認16戦＝304戦、異常・時間切れ・手番打切りなし。
- 回帰: lint、カード166件、PvP5件、Cloud23件、準備/救援11件、`cf:dry-run`成功。
  40枚・最大4枚・他ステージ維持、チヌの投資0/19/20枚境界、現金不足・無属性地禁止を確認。
  2ブラウザ実機対戦・演出の目視検証は今回の範囲外。初回強化のキャッシュv317。

#### 同日追記: クエの封殺・ホライズン追加（キャッシュv318）
- ユーザー指定で`queFinal`の資本主義の権化4→2、言論封殺0→1、ホライズン1→2。
  固定40枚（16/6/18）維持。他のカード・チヌのデッキ・他ステージは変更しない。
- 「クエは土地投資なし／チヌは上げてもLv2ならホライズンの効果はどうか」は
  評価依頼としてローカル比較のみ。**本番AIのレベル制限は未変更**。
  相手のLv3以上の土地は低下、自分のLv1は上昇、Lv2は据え置き。ただし奪った高額地や
  同色のお札にも損失が出るので、既存AIは詠唱200G込みの同盟総資産差が300G以上
  有利になる時だけ使用する。封殺はクエでも既存AIが使用でき、同盟者には撃たない。
- 新デッキ固定・前節と同じ救援ありCPUハーネス、seed4401各12戦、目標24,000G。
  通常AI: 11/12勝、平均155.3手番、ホライズン計17回（`25b0db5ea6477662`）。
  投資制限案: 11/12勝、平均175.3手番、ホライズン計26回（`9b7a70d5623e02fa`）。
  制限案はメモリ内だけでクエの`_cpuMaybeLevelUp`を見送り、チヌの投資上限をLv2に固定。
  土地の獲得・スペル効果は通常のまま。全24戦決着、異常・時間切れなし。
  限られた相手・乱数条件なので勝率差の証明ではない。ホライズンは活きるが、
  自陣とお札の資産成長を抑えるトレードオフがあり、この比較では決着が遅くなった。
- 回帰: 167件（同盟チヌの高額地をホライズンで潰さない判断／クエの封殺対象も確認）、
  lint、PvP5件、Cloud23件、cf:dry-run。戦闘計算・通信・AI本体には手を入れていない。

#### 2026-09-19 独立シードでの再計測（Claude・未対処の要判断あり）
上の調整は seed1101/1701/4401 で詰めた値。**別シード（7777・3407）で素の
`tools/simulate.mjs`を回すと、救援経路が真エンド経路と同じ勝率になっていた。**
条件は `--deckA=@tools/decks/hinanjo-deck2.json --deckB=chinuFinal --allyB=queFinal
--map=ou --goal=24000 --start=1000 --maxTurns=600 --timeout=60000 --games=30`。
`chinuFinal`/`queFinal`のaiProfileは`PERSONA_BY_DECK_KEY`がstory.jsから自動で拾うので
上書き不要。2vs2はサーティーを最初から置く近似（実際の3,000G差の途中参戦より**甘い**）。

| 経路 | seed7777 | seed3407 | 合計 | 主人公側の最終総資産(平均) |
|---|---|---|---|---|
| 真エンド（1vs2・救援なし） | 1/30 = 3.3% | 3/30 = 10.0% | **4/60 = 6.7%** | 4,450 / 5,978G |
| バッドエンド（2vs2・救援後） | 2/30 = 6.7% | 2/30 = 6.7% | **4/60 = 6.7%** | 8,187 / 6,189G |

signature: `0f75c1857005eeab` / `0dbdca89d4e5d021` / `2db4d857a5b5ea3d` / `d53ba02e5f095536`。
全120戦が決着、打ち切り・時間切れ・異常0。

- **真エンド6.7%はユーザー指定「激ムズでいい」どおり**なので触らない。
- **問題は救援経路も6.7%で、サーティーが来ても難度がまったく下がらないこと。**
  旧記録（狩りLv3時代）の2vs2は50%で、救援は「敗色濃厚からの逃げ道」として
  設計されていた。2026-09-20でチヌを`huntMinLandLevel:3→2`にした結果、
  怨念4枚の狩りが主人公とサーティーの土地を序盤から刈り、盤面が戻らなくなっている
  （主人公の最終土地6〜8枚に対し敵は13〜16枚）。
- 効く操作は1つだけ。**チヌの`huntMinLandLevel`を2→3に戻すと、救援経路が
...
#### 2026-09-20 ⑱⑲正式タイトル・救援・お札利回り（Codex、現行）
- ⑱「一騎討ち」、⑲「２人の王」。仮題を廃止。⑱共有マップ名も更新。
- ⑲救援条件を敵陣営合算−主人公の総資産差5,000G以上へ（旧3,000Gを上書き）。
  一度きりの発火、会話、途中参戦の手番処理、エンディング分岐は従来どおり。
- ⑲救援だけ`thirtyFinal`の40枚へ。⑪等の`thirty`は変更しない。
  ⚠️ この時点の構成（Ninja4等の無属性＋雷）は**2026-09-22に森へ全面改装済み**。
  下の「⑲サーティーの森化」が現行。ここの枚数内訳（「モンスター20/装備10/
  スペル10」「フリーランサー2」）は当時から実物と食い違っていた（実際は18/12/10で
  フリーランサーは入っていなかった）ので、数え直す時は`battleCards.js`を見ること。
- OFUDA_LAP_YIELDを8%→16%。お札のある全ステージと対人マップ共通。
  基本/土地ボーナスは据え置き。フリーランサーの1.3倍は16%部分にも従来どおり適用。
  会話・ヘルプを16%に更新（歴史欄の8%は旧仕様）。
- 全クエは帰巣本能と副業収入を両方持ち、使用可能なら帰巣を自分へ最優先。
  CP未回収でもこの組み合わせは帰還する。50G不足/使用済み/スペル禁止は実行不可。
  帰還後に他のスペルやサイコロを処理しない既存の手番冒頭経路を使用。
- simulatorの人物設定採集にmidBattleAssist.allyを追加（救援専用デッキ名/AIの再現）。
- 全自動テスト（cards179/PvP5/Cloud23/story22）・lint/build成功。
  同条件CPU比較: hinanjo-deck2+サーティー vs chinuFinal+queFinal、ou、
  goal24000/start1000/maxTurns600/timeout60000/seed7777、各6戦。
  主人公陣営: 旧thirty 0/6勝→thirtyFinal 2/6勝。12戦全決着・異常0。
  signature旧c4dbe88e6f0c14ef、新72d473be31779d0d。
  最初から4人のCPU対戦であり、5,000G差での途中救援や人間戦の勝率ではない。

#### 2026-09-20 お札の売買UI（全ステージ共通、Codex）
- 属性カード→買う/売る→枚数→同一パネル内で確認/確定。売却は全量固定を廃止。
  ±5、5/10/20枚、半分、最大、直接枚数入力。5枚単位・購入最大50枚・1ターン1回は維持。
- ヘッダーの閉じるボタンをスクロール領域外に固定。横持ち/縦持ちに対応し、
  Escapeでも閉じる。相場閲覧では注文操作を出さない。別モーダルへの確認重ねを撤去。
- `src/ofudaMarketUi.js`がUIを担当。相場payloadのquoteModel（未丸め圧・上下限・単位G）
  に基づく読み取り専用見積もりで、逐次約定後の支払/受取・所持G・枚数・単価を表示。
  実際の売買計算・相場ルールは変更なし。旧payloadでは概算表示へフォールバック。
- 閉じる/取消後と確定連打の遅延イベントを無効化。画面更新は表示だけで財産を変更しない。
- tests/newCardsは上下限/小数売買圧で実売買と見積もりを照合。
  tests/ofudaMarketUi.test.mjs（test:story内）で部分売却・数量・確認戻り・破棄を確認。
  Edgeヘッドレスで844×390/390×844/1366×768のスクロール、閉じる、10枚売却、
  二重確定防止、最大購入、閲覧のみ表示を確認。実iPhone/PvP2端末テストとは別。

#### 2026-09-20 CPU装備・投資・怨念の狩り再調整（Codex、現行）
- ユーザーの狩り強化指定を優先し、⑱⑲チヌのhuntMinLandLevelは1。
  以前の「⑲は3固定」の難易度制限を上書き。救援発火条件自体は変更なし。
  隣の第一候補に勝てなくても、別の勝てる敵を探す。既存の狩り回数・費用・
  勝率閾値・同盟除外は維持。
- 戦闘予測コピーに恒久成長等の個体状態を含める。怨念の成長が消えていた。
  装備の仮定は公開される相手の支払可能アイテム候補から取り、実際の選択は読まない。
  相手にアイテムが無いなら架空の装備を仮定せず、裸で勝てるなら温存する。
- CPU共通のレベル投資は土地HPを受けられる配置かつ残りHP30以上に限定し、
  その中で連鎖・耐久を優先。フリーランサー等の不一致配置には投資しない。
  ⑲クエのみlevelUpElements:[thunder]で雷以外を禁止（他のクエはテーマ維持）。
- ⑲クエ: ホライズン2→遅延行為1・サイコキネシス1、40枚維持。
- クエ共通: 取引所で所持1000G以上なら、本命より安く基礎価格+5G以内の別色へ
  通常予算の1/4以内、最大200G/10枚を分散投資。本命購入と合わせて元の予算内。
  通行料・召喚・塗り替え用の軍資金を残す。任意地点での売買権は追加しない。
- test:all（cards176/PvP5/Cloud23/story18）、lint/build成功。
  動作スモーク: hinanjo-deck2+thirty vs chinuFinal+queFinal、ou、24000G、
  初期1000G、seed7777、4戦全決着・チヌ側4勝・異常0。signature eb79a844d998fd11。
  最初から全員CPUの2vs2。実際の途中救援/人間戦勝率の証明ではない。

#### 2026-09-20 怨念の集合体の属性・画像（Codex）
- ユーザー指定で雷→無属性。定義をneutralMonsters.jsへ移し、IDは維持。
- 専用画像`public/images/card-art/onnenNoShuugoutai.png`を生成・収録。
  プロンプト: 無数の魚の頭骨・背骨・肋骨が蠢く怨念の群体、暗い控えめな背景、
  雷・文字・枠なし。組み込み画像生成を使用。
- HP40/ATK40、先制/貫通、撃破成長、100G、土地コマンド30G、報酬専用は維持。
- 過去のチヌ勝率は雷属性時点の測定。無属性化後の勝率とは区別する。

#### 2026-09-20 ⑲の手札破壊強化（Codex）
- クエ: アイキャンフライ4→2、キャンセルカルチャー0→2。40枚維持。
- ⑲チヌ・クエのみ `cancelCultureDenyOptions:true`。パンデミック最優先を維持し、
  次に人間を優先。同名複数枚を含め1種類しかないスペル/装備を狙う。
  両方1種類なら枚数の少ない方、同数なら装備。他ステージの判断は維持。
- 比較: ひなんじょ本人提供デッキ2（ブリモンは既存の強構成仮定）、ou、
  goal24000/start1000/maxTurns600/timeout60000、seed7777/3407各30戦。
  チヌの狩りLv3・救援条件は変更なし。
  敵勝率: 救援なし54/60→57/60（95%）、最初からサーティーあり38/60→40/60（66.7%）。
  新120戦は全決着・異常/時間切れ/打ち切り0。小標本なので強化幅は断定しない。
  signature: solo 9efe4572a82d17a1 / 32e9d9d5b4bd1595;
  ally 39c54fe65ed6e75f / ab1cfe96a8107b1e。
- 全員CPU操作の比較であり、人間優先分岐は単体テストで別途確認。
  実際の途中救援や人間の打ち筋を再現した勝率ではない。
- lint/build/PvP5/Cloud23成功。カード171件中170成功、既存のエンディングBGM
  ソース検査1件失敗は今回未変更。

#### 2026-09-22 ⑲サーティーの森化（ユーザー指定・現行）
`thirtyFinal`を無属性＋雷から**森**へ全面改装。⑪等の`thirty`は変更しない。

**なぜ森か**: `ou`の土地41マスは火/水/森/雷が各10＋無属性1。雷はチヌもクエも
雷特化で奪い合っている上、サーティーは5,000G差がついてから参戦するので着いた
時には残っていない。火/水/森は敵が触らない。森を選ぶと更に2枚が使える:
- **森神の盾**(80G): 森属性の反射は`unpierceable`で**怨念の貫通では消せない**。
  無属性デッキには組めない怨念対策で、不死鳥の盾より10G安い。
- **社会不適合**(70G): 土地と属性が合わないモンスターを手札へ返す。怨念は無属性で
  無属性マスが1枚しか無いのでほぼ常に該当し、撃破成長(`lapGrowthAtkBonus`は
  個体の値)が**40/40へリセットされる**。育った怨念を初期化できる唯一の手段。

**40枚（19/12/9）・同一カード最大3枚（ユーザー指定）**
- マタギの小四郎3・樹海の怨霊3・暴れイノシシ3・巨珍兵3・山賊フクロウ3・
  枯れ木の巨人2・くぐつの剣豪1・専用ブリモン1
- 森神の盾3・ライフジャケット3・人食い草2・異次元ソケット2・不死鳥の盾1・
  イカサマのサイコロ1
- 帰巣本能3・社会不適合3・キャンセルカルチャー3

**計測（`hinanjo-deck2`＋サーティー vs `chinuFinal`＋`queFinal`、ou、
goal24000/start1000/maxTurns600/timeout60000、seed7777・3407・5150で各30戦）**

| 構成 | 7777 | 3407 | 5150 | 合計 |
|---|---|---|---|---|
| 旧`thirtyFinal`（無属性＋雷・資金1000G） | 20.0% | 16.7% | 16.7% | 17.8% |
| 森・高コスト連鎖型（山神/世界樹/徴税官） | 26.7% | 10.0% | — | 18.3% |
| 森・低コスト型のみ（資金1000G） | 23.3% | 13.3% | — | 18.3% |
| 旧デッキ＋資金2000G | 16.7% | 30.0% | — | 23.3% |
| **実装版（森＋資金2000G）** | **36.7%** | **40.0%** | **23.3%** | **33.3%** |

signature（実装版）: `0a93b1f5017f0d30` / `7acbc1159d84507e` / `e8e1b8e2769904e4`。
計270戦＋検討用360戦、全て決着・異常/時間切れ/打ち切り0。

**⚠️ 効いたのはデッキではなく資金の方が大きい。** 森デッキ単体では17.8%→18.3%で
誤差。2,000Gを付けて初めて33.3%になった。**`ally.startingCurrency`を消すと
改装の効果はほぼ消える。**

**⚠️ 同時に直した不具合**: `main.js`の`assistConfig`が`startingCurrency`を
渡しておらず、途中参戦の味方は`game.js`の既定値**500G**で参戦していた
（ステージの`startingCurrency` 1,000Gすら届いていない）。通常の`ally`と同じ
`allyDef.startingCurrency ?? stage.startingCurrency`に揃えた。⑪のサーティーも
この経路なので、そちらも500G→ステージ値に変わる。

**触るときの注意**
- 高コスト・連鎖前提の森（山神150G・世界樹120G・森林徴税官90G＝要2連鎖）は
  計測で現行以下。サーティーは土地が3〜5枚で頭打ちなので連鎖が立たない。
  **安いカードで枚数を出す構成を崩さないこと。**
- **断線事故**（全雷モンスターに20ダメージ）は敵2人とも雷・こちらは森で同士討ち
  ゼロという理想の1枚に見えるが、`target:'none'`の全体ダメージ系はCPUの使用判断が
  無く**一度も詠唱されない死に札**（`_cpuMaybeUseMismatchedLandDamageSpell`の
  コメント参照）。入れるなら先にCPU判断を実装する。
- サーティーの最終土地数は**どのデッキでも3.1〜5.6枚から動かない**。参戦が遅くて
  土地が残っていないのが根本で、デッキで直せる範囲を超えている。更に上げるなら
  救援条件（5,000G差）か参戦資金を動かすこと。
- チヌの`huntMinLandLevel`は1（2026-09-20のユーザー指定）。上の数字は全てLv1前提。
...
### まだ未定
- ~~⑱の目標総資産~~ → ✅ **確定22,000G・両者500Gの対等スタート**（「⑱の数値調整」）。
  ⑲の目標総資産は未定。~~⑲の盤面~~ → ✅ **⑱の`ou`をそのまま使う**（ユーザー指定）。
- ⑲でクエが既存デッキ（`que`/`queKessan`）のままか、決戦用に強化するかも未定。
- ~~⑱の盤面の形~~ → ✅ **確定・実装済み**（下記「⑱の盤面とデッキ」）。
- ~~チヌの専用デッキ~~ → ✅ **確定・実装済み**（同上、`battleCards.js`の`chinu`）。
- ~~川田はここに参加しないのか~~ → ✅ **解決**。⑰のoutroで川田が
  「配下は俺たちに任せろ」と労働施設に残るため、⑱⑲に居ない理由が
  作中で説明されている（⑰の項参照）。
- ~~⑱の会話~~ → ✅ ユーザー指定全文をstory.jsへ反映済み。⑲の会話と、クエがチヌ側に付く動機は未定。
- **⑱のステージ名**。`board.js`のMAPS名は`'⑱ 王の間（仮題）'`で仮置き。
- **⑱の専用背景**。⑯の`king-room.png`を流用中。

### ⑱の盤面とデッキ（2026-09 実装済み）
ユーザー指定は「**盤面は『王』の字**」「**CPは4つまで**」「**EXカード、ふんだんに
使っていいよ。王だから**」「**酢は入れていい。資本主義の権化でばらまこう**」。

**盤面** `board.js`の`OU_ROWS` / MAPS `id:'ou'`（`wip:true`＝対戦モードには出さない）。
15列×14行・**実マス46**。2026-09-19のユーザー指定で横棒3本・縦棒を1マス幅へ細線化。
旧94マス版は棒が厚く隣接・分岐が過多だったため廃止。端6か所では既存の折り返しを使う。
全マス到達可能、分岐は横棒と縦棒の交点3か所。
- CPは4つ。上の横棒の左端(0,2)・右端(0,12)、**中央の玉座(6,7)**、下の横棒の
  右端(13,14)。スタートは下の横棒の左端(13,0)。`requireAllCheckpoints`と合わせて
  「王の字を端から端までなぞらないとゴールできない」形。
- 属性は火水雷森が各10マス＋無属性1マス、**最大連鎖3**。
- ⚠️ **素の連鎖を3で抑えてあるのは前提**。チヌの国士無双！！が「連鎖数×4」なので、
  ここを緩めると放水を撃つ前から跳ねる。

**BGM（2026-09-19）** ユーザー提供の `public/audio/stage18bgm.mp3` を
`TRACK_SRC.kingDuel` として登録。`story.js`の⑱に `bgmTrack:'kingDuel'` を設定し、
`startStoryBattle`から既存のBGM上書き機構へ渡す。初戦・再戦・セーブ再開と戦闘後の
盤面曲復帰に適用。⑲と同じmapId `ou`なので `MAP_TRACK.ou` には設定しない。
⑲・エンディングの既存曲は保持。対戦のBGM選択では「♪ステージ18」。キャッシュv310。

**デッキ** `battleCards.js`の`chinu`（⑯の`chinuUsagin`等とは別物。取り違え注意）。
固定40枚（モンスター21／アイテム12／スペル7）、EXは言論封殺1（チヌ専用、下記）・
資本主義の権化1・ペーの杖1。**2026-09に「合法ミラー」構成へ差し替えた**（「⑱の数値調整」⑨）。
ユーザー指定は「**両者500Gの対等スタート**」「お札ありの長期戦」「**合体系（ガシャーン）
だけ禁止、単にEXなだけなら可**」「属性は自由」。
- **主人公役デッキ（`@tools/decks/hinanjo-deck2.json`）の合法ミラー**: 雷の先制トリオ
  各4＋Ninja4＋イカサマ4・ナンカ4・真剣白刃取り2・ペーの杖・不死鳥の剣。ブリモン
  （ブリード品でカタログに無い）だけくぐつの剣豪1で代用し、千本桜を言論封殺1にした。
  くぐつ3・放電2・国士2の「エンジン型」(A3)は43%でこれより弱かった。
- **チヌが雷なのは意図的**。fixerのお札は「その属性の土地レベル合計」で値上がりするので、
  雷の主人公が雷地をレベルアップするたびにチヌの雷お札が勝手に上がる（ただ乗り）。
  火・森の同型は5%まで落ちた。
- ⚠️ **`aiProfile:{ofudaStyle:'fixer'}`は必須**。無いと同seedで50%→25%。
- 「王は戦わない、海を塗り替える」の水単色EX13枚版（放水4・酢2）は、対等500Gだと
  一枚も撃てず**0/20**で没。
- **EXの追加は益なし**（目標22,000・対等500G・seed21/41/51×20戦）: 現行28/60、
  ペーの杖2を足して26/60、ペー1＋酢1を足して18/60。**酢は300Gが手札で腐って
  -16.7pt**（約1.8σ）。EXは資本主義の権化2・国士無双2のままにしている。

### チヌ専用EXスペル「言論封殺」（2026-09 ユーザー指定・実装済み）
`battleCards.js`の`SPELL_CATALOG.genronFuusatsu`。EX・100G・対象は相手プレイヤー
（`target:'enemyPlayer'`）。効果`{type:'spellBanCurse', turns:3}`＝**3ターンの間
スペルを使えなくする呪い**。`rewardOnly`でショップには並ばず、画像は未用意
（`imageDataUrl:null`、共通絵へフォールバック）。チヌのデッキに2枚（千本桜1・
財布チューチュー1と交換）。
- **プレイヤー呪いの1枠を使う**（`_applyPlayerCurse(target,'spellBanTurnsRemaining',3)`）。
  相手の脱税・宝くじ・タフネス等の有益な呪いも上書きして消す（不可侵ルールどおり）。
  逆に相手が後から別のプレイヤー呪いを受ければ封殺も消える。
- 自分・同盟仲間には効かない（`_applySpellEffect`の`spellBanCurse`分岐で弾く）。
- 止める場所は2つ: 人間側`useSpell`（ログ「言論封殺中はスペルを使えません」）と、
  **CPUの唯一の詠唱経路`_cpuCastSpell`の先頭**。個別の`_cpuMaybeUse*`が見落としても
  ここで必ず止まる。CPUのターン頭の`canUseSpells`にも足してあるので、封殺中は
  スペル群のルーチン自体を飛ばす。
- 減算は**対象本人の手番終了時**に1（`_nextTurn`、ハッキングと同じ扱い）。
  相手の手番では減らない。解けた時にログ「言論封殺が解けた」。
- PvP: `publicState.players[].spellBanTurnsRemaining`で配り、ゲスト側の手札描画は
  `renderHand`の使用可フラグを落とす（ホスト側`useSpell`でも二重に弾く）。
- CPU使用判断`_cpuMaybeUseSpellBanSpell`: 手札にスペルを最も多く握る敵（同盟外・
  未封殺）へ撃つ。**相手がスペルを1枚でも持っていれば撃つ**（ユーザー指定）。
  0枚なら温存。呼び出しは国士無双の直前。
- ⚠️ **勝率は下がる**（対等1,000G・目標22,000・seed21/41/51×20戦）: 入れる前26/60
  （43%）→ 入れて15/60（25%）。「余裕資金300Gを残し相手が2枚以上の時だけ」に絞っても
  17/60で変わらず、抜く札をアイキャンフライ2にしても同seedで同値。**ユーザーの
  判断で条件を付けずに撃つ仕様を採っている**ので、これを理由に外したり絞ったり
  しないこと。⑲（チヌ＆クエ）の調整で必要ならクエ側で補う。

### チヌ専用モンスター「怨念の集合体」（2026-09 ユーザー指定・実装済み。名前もユーザー指定＝「殺された幾つもの骨の魚達が蠢く」怨念の塊）
`thunderMonsters.js`の`onnenNoShuugoutai`。雷・EX・100G・**40/40・先制・貫通**、
...
- コスト（100G仮置き）と画像はユーザー確定待ち。画像の指示: 殺された幾つもの骨の魚達が蠢く集合体。
...
（各独立seed5本×20戦）。1,000Gスタートでは現行デッキで26/60（43%、seed21/41/51）と
500Gの28/60と誤差圏＝**開始資金はバランスに効かない**。言論封殺2枚を入れた後の
再計測は上の「言論封殺」の節（入れて25%）。

計測は `npm run simulate`。主人公役は⑰と同じ `@tools/decks/hinanjo-deck2.json`
（CPU同士なので「AIがそのデッキを回した時の強さ」であることに注意）。
simulate.mjsへ`--startA/--startB/--startA2/--startB2`（座席別の初期資金）を追加してある
（ゲーム側のコードには触っていない）。

**① 「国庫」案は却下された。** 最初は水単色EX13枚のデッキが500Gでは0%だったため、
チヌの初期資金を13,000Gにして53.8%へ持っていったが、ユーザーから「**普通に対等の
条件でスタートしないとゲームとしておかしい**」で却下。初期資金の補正は
stageにもopponentにも置かない（回帰テストが`startingCurrency`の不在を見ている）。

**② 対等500Gで立ち上がるのは雷テンポ型だけだった。** 同seed21・20戦・fixer付き:
水単色EX13枚版 0/20、無属性くぐつ型 0/20、水テンポ型 1/20、**雷テンポ型 10/20**。
差は「30〜50Gの先制持ちを12枚積んでいるか」。高価な放水150G・国士無双120G・
酢300Gは500Gスタートでは一枚も撃てず、土地を取る前に負ける。

**③ fixerは必須。** 雷テンポ型で fixer無し 5/20 → fixer有り 10/20（同seed）。
⑰と同じく、効くAI設定はこれだけ。

**④ 目標総資産は効かない。** 雷テンポ型＋fixer、独立seed 21/41/51/61/71×20戦:
18,000G → 41/100、22,000G → 38/100（25,000Gはseed21で8/20）。長期戦にしても
主人公側に置いていかれないので、ユーザー指定の「長期戦想定」どおり22,000Gを採用。

**⑤ 変種の比較（seed21）。** 放電3・国士3へ寄せてアイキャンフライを抜くと35%へ悪化
（安い出目操作を削ると序盤が鈍る）。くぐつ3・落雷予報士2（採用版）はA相当の
10/20、目標22,000では13/20とAの12/20以上。

**⑧ 言論封殺は2枚→1枚**（ユーザー指定）。2枚入りは15/60（25%）だった。

**⑨ 「プレイヤー勝率20%」を目標に総当たり（対等1,000G・目標22,000・fixer、seed21×20戦）**:

| 構成 | チヌ勝率 |
|---|---|
| ⑬クエ(queKessan、最適化・相場型) | 1/20 |
| 火テンポ型（ヘチマ竜4等） | 1/20 |
| 森テンポ型（樹海の怨霊4等） | 1/20 |
| くぐつ4特化 | 2/20 |
| 関所クラゲ3＋アリジゴク | 2/20 |
| 火＋貫通スウォーム（未知の侵略者4） | 2/20 |
| ⑭塞ぎ込んだ男(fusagikonda、パンデミック金策) | 3/20 |
| A3＋放電4 | 3/20 |
| ⑮川田(kawada) | 8/20 |
| A3（雷テンポ、旧採用） | 8/20（5seed合算43%） |
| **ミラー（主人公役デッキそのまま＋fixer）** | **13/20** |
| **H1 合法ミラー（採用）** | 11/20、**5seed合算 49/100** |
| H1＋国士無双2 | 10/20 |

**結論: デッキだけではCPU同士で50%前後が天井**。主人公役のデッキをそのまま持たせた
ミラーでも65%止まり。「プレイヤー勝率20%」＝チヌ80%は、しかも実プレイヤーは
主人公役のCPUより上手いので、**対等条件のデッキ調整では届かない**。
...
**⑩ 怨念の集合体＋狩りAIの調整（対等1,000G・目標22,000・fixer）**:
...
`_cpuMaybeUseAppraiserSpell`と`_cpuMaybeUseLandSpell`の2経路が実装済みで、実戦ログで
詠唱を確認済み）。ただしプレイヤー呪いなので、撃つとタフネス等の他の呪いを
上書きして消す。雷テンポ型には入れていない。
⚠️ **カードが「CPUに使えるか」はコメントではなく`game.js`の実装で確かめること。**

⚠️ **seedによる振れが非常に大きい盤面**（同条件でseedごとに25%〜60%まで動く）。
n=20での比較は意味が無い。最低でも独立seed×5＝100戦で判断すること。
...
概要:
- **モンスター19**: アイランドホエール×3(成長型の壁)・ゆきおんな×3
...
- **アイテム11**: 水神の盾×3・異次元ソケット×2・ツインハンマー×2・
  ...
  水神の盾は2→3に増量。
- **スペル10**: ファイヤーボール×2・バックファイア×2・放水×1・
...
#### 資本主義の権化は採用のまま確定（2026-09、ユーザー判断）

⚠️ **一度撤去したが差し戻し、「いまのままでいい」で確定した。もう動かさないこと。**
経緯を残しておかないと三度目の往復が起きるので、両論を併記する。

- **撤去した理由（2026-08）**: ⑫でクエを破産させた時だけ手に入る**クエ専用のEX**で、
  ⑫のストーリーでも「クエが去った跡に残されていた」と明示されている。裏路地の
  レジスタンスである川田が金融街のフィクサーの切り札を撃つのは演出として
  筋が通らない、というユーザー指摘（「⑮に全く関係ないクエが登場する」）を受けて
  外した（コミット`0c01460`）。
  - ⚠️ ただし**この札はセリフを喋らない**ので、「クエが喋る」という報告の
    原因ではない。その件は別途未解決（上の「⑮に関係ないクエが登場して喋る」節）。
- **差し戻し（コミット`f05dc6a` "Restore capitalism spell to Kawada deck"）**の後、
  ユーザーが**「いまのままでいい」と明言**（2026-09）。よって**採用が最終決定**。
  撤去時に入れた回帰テスト「川田のデッキに他キャラ固有のEXカードを混ぜない」は
  差し戻しの際に削除され、逆に**「資本主義の権化を1枚含める」ことを要求する
  テスト**へ差し替わっている。デッキを触る時はそちらが基準。

**勝率上はどちらでも誤差**（判断材料として残す）:
- 採用時の計測: 資本主義あり155勝/480(32.3%) vs なし146勝/480(30.4%)。
  差+1.9ptに対し差の標準誤差が約2.9ptで有意ではない。
- 撤去を検討した時の差し替え候補比較（4シード×120戦＝各480試合）:

| 構成 | 勝率 |
|---|---|
| **資本主義の権化1枚（現行）** | **178/480 (37.1%)** |
| → アイランドホエール4枚目 | 176/480 (36.7%) |
| → 混沌の頭を戻す | 162/480 (33.8%) |
| → 逆流カジキ | 157/480 (32.7%) |
| → くぐつの剣豪4枚目 | 153/480 (31.9%) |

  アイランドホエール4枚目だけが実質同値（2勝差、-0.4pt。差のSEは約3pt）で、
  他3案は5pt前後の明確な悪化。**もし将来また外すなら代替はアイランドホエール一択**。

札の挙動: 召喚条件と生け贄を無視して手札のモンスターを安い順に空き地へ連続召喚する。
水連鎖1が要る水神/関所クラゲ/水風呂修行僧を条件未達でも出せるのが噛み合っている。
CPU判断は`_cpuMaybeUseCapitalismIncarnateSpell`（空き地2つ以上＋2体以上
並べられる時）で、実測1試合3〜9回発動。`rewardOnly`はNPCデッキ構築では
除外されないのでそのまま持たせられる。枠は実測最低効率だった混沌の頭
(150G 30/30 = 0.40 pts/G)を撤去して確保したもの。
...
#### CPUの対象選択の優先順位（2026-09、ユーザー指定・CPU全体の共通挙動）
- **ダメージ系スペルは「くぐつの剣豪」を最優先**（`DAMAGE_SPELL_PRIORITY_IDS`）。
  くぐつは`autoInvadeEachTurn`で毎手番こちらの土地へ自動侵略し続け、
  `immovableByMoveCommand`で動かすこともできない。避雷針侍・くねくねが
  「置かれている間だけ侵略を妨げる」受け身のロックなのに対し、くぐつは
  能動的に盤面を削り取ってくるので一段上に置く。
  - ⚠️ **この優先はkillable判定より前に効かせている**。くぐつはHP50で
    ファイヤーボール15では落ちないので、「倒せる相手が居ればそちらを優先」の
    後ろに置くと、**一撃で倒せる雑魚が1体でも居る限り永久に狙われない**。
    順序を入れ替える時はここに注意すること。
  - くぐつが複数居る時は、その中で倒せる（＝既に削れている）方を仕留める。
- **キャンセルカルチャーは「パンデミック」を最優先**
  （`CANCEL_CULTURE_PRIORITY_IDS`）。パンデミックは盤面のモンスターを全て
  ゾンビ(HP20/ATK20)へ置き換える＝積み上げた盤面を一撃で更地にするので、
  通した時の損失が他のどのカードより大きい。優先順位は
  **パンデミック ＞ アイテム ＞ その他のスペル**（アイテム優先は2026-09-01の
  別のユーザー指定）で、同じ段の中では高コスト順。
  - ⚠️ **対象プレイヤーの選定にも効かせている**。総資産1位だけを見ていると、
    パンデミックを握っているのが別の相手だった時に永久に抜けない。
- 回帰テスト: `npm run test:cards`「ダメージスペルのCPUは避雷針侍・くねくねを
  最優先で狙う」（くぐつの段を追記）／「キャンセルカルチャーはアイテムより先に
  パンデミックを潰す」／「パンデミックを持つ相手が総資産1位でなくても、
  そちらを狙う」。いずれも優先セットを空にすると落ちることを確認済み。
  - ⚠️ テストヘルパの`asItem`/`asSpell`は**`catalogId`を持たせている**。
    実物のデッキは`duplicateForDeck`が`catalogId`を付けて`id`を振り直すので、
    `catalogIdOf`（`catalogId ?? id`）でカード種別を引く判定はヘルパ側にも
    `catalogId`が無いと素通りしてしまう。
