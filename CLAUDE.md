...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v258`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
### ⚠️ 未解決: 主人公の一人称が⑮だけ違う
- ①〜⑭は一貫して「**俺**」（28回）。⑮だけ「**ワイ**」＋関西弁
  （「ワイは主人公」「考えるんや」「後付けでええやろ」）。
- ⑮の台詞はユーザーが逐語指定したもので、**⑯⑰の未実装スペックも「ワイ」**
  （「ワイは王になりにきた」「ワイのせいで…めんぼくねぇ…」）。
- つまり⑮以降で意図的に口調を変えたのか、書き分けの揺れなのかは**未確認**。
  意図的なら①〜⑭は据え置きでよいが、揺れなら⑮以降を「俺」へ寄せる必要がある。
- なお③⑧⑪にも「話はそれからや」「かかってこいや」等の関西寄りの語尾はあるが、
  **一人称は「俺」のまま**なので、口調が丸ごと変わっているのは⑮だけ。
