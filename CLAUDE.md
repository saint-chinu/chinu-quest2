...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v251`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
- ⚠️ **この反射は相手の貫通では無効化されない「絶対反射」**（ユーザー指定、
  2026-08）。`equipItem`が`{type:'reflectDamage', unpierceable:true}`を立て、
  `dealDamage`が`reflect.unpierceable`なら貫通を無視して反射する。
...
- ⚠️ **差し替え後も`wielderElement`を残し、`dealDamage`で「今その装備を持って
  いるモンスターの属性」を再判定する**。装備時にreflectDamageへ差し替える
  実装なので、これが無いと**真剣白刃取りで奪われた／異次元ソケットで効果を
  移された属性の合わないモンスターまで絶対反射を得てしまう**（2026-08に発見・
