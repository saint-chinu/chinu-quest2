...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v314`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
救援会話は「クエを追ってきたら…」から「チャンスだ！」まで指定4行に差し替え。
`midBattleAssist.overlay`で主人公を左・サーティーを右に表示し、⑪の闇ホフクを流用しない。
...
- **サーティー参戦**: `midBattleAssist.enemyAssetsLeadAtLeast: 3000`。敵陣営の合算総資産が
  ...
  真エンド（助け無し）を狙えるバランスにするなら、差の値を上げるか「主人公が1周
  してから判定」等の猶予が要る。**未計測・ユーザー判断待ち**。
...
- **真エンドは「激ムズでいい」（ユーザー指定）。** 人間の攻め筋はパンデミックなので、
  ⑲のチヌは決戦版デッキ`chinuFinal`（⑱からエレキ輝1枚を怨念の集合体へ変更し、
  怨念の集合体3枚・エレキ輝3枚。アイテムは⑱と同じ。合計40枚を維持）に
...
- **計測（目標24,000・両者1,000G・seed21/41×20戦、主人公役はひなんじょデッキ）**:
