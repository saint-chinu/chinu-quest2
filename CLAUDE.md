...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v290`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
固定40枚（モンスター20／アイテム8／スペル12）、EXは資本主義の権化2・国士無双2の4枚。
...
**結論: 目標総資産22,000G・両者500Gの対等スタート・チヌは`ofudaStyle:'fixer'`。
敵側勝率は目標18,000で41/100、22,000で38/100（各独立seed5本×20戦）。**
