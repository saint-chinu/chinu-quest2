...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v233`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
## 新ストーリー「川田」構想（2026-08、story.js未実装・仕様のみpush）
⑭「王都の番人？？」の次、王都に入った直後のステージ⑮として構想中。
**デッキ・新アイテムはコード実装済み**（下記）。**story.jsのステージ本体
（intro/outro会話・opponents配列・goalCurrency等）はまだ未実装**——ここが
「不足分」。Codexが空いていればdialogueは下のドラフトをベースに書き起こして
良い（ユーザーからの最終OKはまだ無いので、大筋を変えない範囲で）。

### キャラクター設定
- **川田**: ビーバーだが本人はマーモットだと信じて疑わない。「俺は是々非々や」
  が口癖の生真面目な議論好き。王都のレジスタンス所属で王政に懐疑的。
  主人公に「何のために王を目指すのか」と問い、「王になってから考える」という
  答えに「面白いやつだな。王の資質があるか、真のマーモットの俺が見定めて
  やる」と言って対戦開始（バトル自体は**純粋な殴り合い**＝1vs1、専用の
  妨害トリック無し、というユーザー要望を反映）。
- 対戦後、川田は是々非々の立場を明言（王政にもレジスタンスにも肩入れしない）
  しつつ、「玉座に近づくほど筋が通らなくなる」と示唆——これは次の展開
  （下記）への伏線。レジスタンス所属＝反王政という立場上、後のステージで
  再登場させて主人公の味方に回す扱いが自然（地下労働施設での再会・脱獄
  協力など）。
...
概要:
- **モンスター20**: アイランドホエール×2(成長型の壁)・ゆきおんな×3
  (全水属性ATK+30オーラ+先制)・半魚人×2/ファイヤーマン×1(先制アタッカー)・
  リャンメンすくな×2(2回攻撃、オーラで実質倍化)・関所クラゲ×1・まんぼー×1・
  ビッグマーメイド×1・水神×1(水連鎖×7)・くぐつの剣豪×3(自動侵略、無属性
  だが採用)・混沌の頭×2(全連鎖×5)・くねくね×1(反射)。
- **アイテム10**: 水神の盾×2・異次元ソケット×2・ツインハンマー×2・
  真剣白刃取り×1・ナンカのお守り×1・ライフジャケット×1・ダイヤモンドの盾×1。
- **スペル10**: アイキャンフライ×2・バックファイア×2・鋼体×2・放水×1・
  青色の魔法陣×1(デッキの水属性をランダム召喚)・占術×2。
- 回帰テストは`npm run test:cards`（`buildCharacterDeckList('kawada')`が
  ちょうど40枚であること）。AIプロファイル（story.jsのopponents[].aiProfile）
  は未設定——ステージ本体を書く時に是々非々の気質（優勢なら畳みかけ、
  劣勢なら守りに回る、くらいの単純な切り替え）を検討する。
