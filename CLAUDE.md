...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v263`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
## ⑯「魚群の王チヌ」構想（2026-08 ユーザー指定・**未実装／仕様のみ**）
⚠️ **まだ一行もコード化していない。** ユーザーから「まだ実装せずに検証と記録
だけ」と指示があったため、下記は仕様と実現可能性の調査結果のみ。着手する時は
このセクションを起点にすること。
...
### 対戦仕様（ユーザー指定）
- **1vs3**（主人公 vs ウサギン・ムール・邪神ヒトデマソ）。**主人公は勝てない
  負けバトル**。目標総資産5,000。
- **ターン順はランダムではなく主人公が最後**。
- 敵3体は**通常のusagin/muuru/hitodemasoデッキではなく専用構成**:
  - 全員**バックファイアを初期手札に1枚ずつ**保有し、**主人公が半周するたびに
    交代で撃って**ゴールさせない。
  - 全員**ファイヤーボール4枚・千本桜3枚**でとにかく主人公のモンスターを破壊。
  - 安いモンスターをガンガン撒いてガチガチに守り、**アイキャンフライ・
    帰巣本能**で自分たちは稼ぐ。
...
### 妨害の発動条件（ユーザー指定）
- **バックファイア**: ①主人公が**CPまで3マス以内**に到達した時、または
  ②主人公が**CP通過後にゴールへ5マス以上近づいた**時に発動。3体が交代で撃つ。
- **1のダイス**も各自**2枚**搭載（`diceOne` N30G `setNextDice` value:1）。
  とにかく主人公をゴールさせない。
...
**そのまま使えるもの**
- 1vs3の陣営分け: `heroAllianceId`/`enemyAllianceId`が既にある
  （main.js `buildBattlePlayerConfigs`）。敵3体に同じenemyAllianceIdを振ればよい。
...
**新規に作る必要があるもの（未着手）**
1. **ターン順の固定**: `this.currentPlayerIndex`は
   `tutorialMode ? 人間のindex : Math.floor(Math.random()*players.length)`
   （game.js 507付近）。「主人公が最後」は人間のindex+1から始める必要があり、
   ステージ側から指定できるオプションが要る。
2. **強制敗北の仕組み**: 現状**存在しない**。「主人公は勝てない」をどう表現するか
   （目標到達を握り潰す／敗北でもクリア扱いで進行、等）は設計判断が要る。
   ⑮で使った`_checkGoalAchievement`のチュートリアル用抑止が参考になる。
   なお**負けてもクリア扱い**なので、`onStoryBattleEnd({won:false})`でも
   進行を保存する必要がある（チュートリアルの「敗北しましたが完了扱い」と同型）。
3. **全員の初期手札にバックファイア1枚**: 初期手札の指定は
   `_prepareTutorialOpeningHand`（tutorialMode限定）しか無い。非チュートリアルでも
   使えるように一般化が要る。
4. **バックファイアの交代発動AI**: 上記の条件判定（CPまで3マス以内／CP通過後に
   ゴールへ5マス以上接近）と、3体が順番に撃つ交代管理を新規に書く必要がある。
   既存の`_cpuMaybeUseDisruptionSpell`等は「撃てる時に撃つ」で交代制が無い。
5. **開始時の大きな2行バナー**（「イベントバトル！！／何があっても諦めるな！」）:
   `onGoalAchieved`→`promptGoalAchieved`のような全画面演出に倣って新設する。
