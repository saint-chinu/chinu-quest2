...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v271`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
- **初期手札からスペルを撤去（2026-08、追加のユーザー指定「最初手札にスペルが
  ...
  - 初期手札を`['fireStarter','knife','potLid','kunekune']`＝**モンスターと
    アイテムだけ**にした。
  - **占術は使う手番にちょうど配る**。`tutorialDrawQueues.human`を
    `['flameLizard','divination','sekizou','molotovMan','potLid','iCanFly']`と
    **全手番ぶん指定**した。⚠️ nullを残すと山札の先頭（＝台本が教えない
    2枚目の占術・アイキャンフライ）が早い手番で来て元の状態に戻る。
  - **占術のステップを自4から自2の頭へ移した**。自2は出目1でCPにちょうど
    停止するだけの手番なので、占術が何を引いても以降の台本は変わらない
    （ユーザー指定「結果に関係ないどうでもいい場面で」）。自4は着地マスも
    召喚も決まっている「結果に関わる手番」だった。移動の副次効果として、
    自4で占術40G＋火炎瓶男50Gを同じ手番に払う必要も無くなる。
  - **1のダイスをデッキから撤去**（台本が一度も案内していなかった）。
    枠は石像へ。レッスン「手札とスペル」の本文からも削除した。
    デッキのスペルは占術2・アイキャンフライ2の**4枚だけ**。
