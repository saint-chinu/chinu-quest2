...
**CPUが目標達成寸前になったら**、盤面を止めて以下の会話を挟み、その後再開する:
...
- **盤面途中のイベント会話には前例がある**: `stage.midBattleAssist` →
  ...
  「敵の総資産が主人公の`ratio`倍を超えたら会話を出す」という作りなので、
  発火条件を「CPUの総資産が目標の◯割」に変え、味方参戦の副作用
  （`_addStoryAssistPlayer`）を外せば流用できる。
