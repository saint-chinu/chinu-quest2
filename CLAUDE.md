...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v289`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
⑰でムールとダンボール男を倒した後の決戦。
**⑱は盤面とチヌのデッキまで実装済み**（2026-09、下の「⑱の盤面とデッキ」参照）。
story.jsのステージ定義（会話・目標総資産・AIプロファイル）はまだ無い。
**⑲は未着手／仕様のみ。**
...
### まだ未定
- ~~⑱の目標総資産~~ → ✅ **確定18,000G＋チヌの国庫13,000G**（上の「⑱の数値調整」）。⑲の目標総資産は未定。
  ⑲の盤面（⑱の`ou`を使い回すのか、別に作るのか）は未定。
...
固定40枚（モンスター16／アイテム5／スペル19）、うち**EX15枚**。
コンセプトは「王は戦わない。海を塗り替える」＝**放水4枚で盤面を水地へ塗り替えて
連鎖を伸ばし、国士無双！！3枚（連鎖数×4のATK/HP上昇）で跳ねさせる**。
- ⚠️ **放水と国士無双はセット**。CPUの`_cpuMaybeUseChainStatCurseSpell`は上げ幅
  10未満（＝連鎖3未満）だと温存するので、放水を削るとEXが丸ごと死に札になる。
- ⚠️ **採用したEXは全てCPUの使用判断が実装済みであることを確認して選んだ**
  （`_cpuMaybeUseChainStatCurseSpell` / `capitalismIncarnate` / `disclosureRequest` /
  `summonBaseHpBoostCurse` / `cashOutOwnLand`）。EXを足す時は必ず同じ確認をすること。
- 酢(EX 300G)は`_spellCapitalismIncarnate`で撒く前提。同関数は**コストの安い順**に
  手持ちGが続く限り撒くので、300Gの酢が出るのは安い順に払い終えてなお300G残って
  いる時だけ。安いモンスター（アイランドホエール40G・50G帯）を厚めにしているのは
  弾を絞らないため。撒き先は**そのモンスターの属性と一致する空き地を優先**するので、
  水単色だと撒くほど水地の連鎖が育ち、放水／国士無双と噛み合う。
- モンスターは**水単色**。⑮で追加した水神の盾の絶対反射が全員に乗る初めての構成。
- 灰塵と未知との遭遇は無属性モンスター前提なので水単色では不発、不採用。
  合体ロボ・ガシャーンも無属性なので不採用。

### ⑱の数値調整（2026-09、シミュレータで計測）
**結論: 目標総資産 18,000G ／ チヌの初期資金（王の国庫）13,000G。敵側勝率53.8%（n=80）。**
story.jsを起こす時は `goalCurrency: 18000` と、opponents側へ
`startingCurrency: 13000` を置くこと（main.jsが
`opponent.startingCurrency ?? stage.startingCurrency` で座席別に効かせる）。

計測は `npm run simulate`。主人公役は⑰と同じ `@tools/decks/hinanjo-deck2.json`
（CPU同士なので「AIがそのデッキを回した時の強さ」であることに注意）。
座席別の初期資金を振れるよう simulate.mjs へ
`--startA/--startB/--startA2/--startB2` を追加した（ボス補正の検証用。
ゲーム側のコードには触っていない）。

**① 初版のデッキは動かなかった（勝率0%）。**
安いモンスターが8枚しか無く、「安物を引けない→土地が取れない→収入が無い→
高いEXが撃てない」の死のスパイラルで**土地1.3枚・総資産608G**。初期資金を
2,000Gへ上げても土地4.6枚で0%のまま。**30G以下の安物（泥酔クラゲ3・冷やし中華2・
バケツリレー隊2）を7枚入れて土地20枚まで回復**した。氷柱・海の家店長・半魚人・
真剣白刃取り・千本桜と、タフネス／開示請求の2枚目を落として枠を作っている。
⚠️ **ここの安物を削ると同じスパイラルへ戻る**（回帰テストが7枚を下限で見ている）。

**② AIプロファイルは一切効かない。**
`levelUpReserve` / `minWinProbabilityToInvade` / `highValueAvoidance` を振っても
**同一seedで結果が完全に一致**した。チヌが終始金欠でどのダイヤルも一度も条件に
届かないため。⑮⑰と同じ結論（効くのは経済エンジンだけ）。
よって⑱のaiProfileは**無指定でよい**。

**③ 目標総資産は上げるほどチヌが弱くなる。** 長期戦は主人公側のエンジンが勝つ。
国庫6,000G固定で goal 18000→15.0% / 25000→15.0%、国庫10,000Gで 18000→30.0% /
25000→0.0%。**25,000Gは論外**。

**④ 効くのは国庫だけ。** goal 18,000G固定、seed 41/51/61/71×20戦＝80戦:
...
おおむね1,000Gあたり6pt。ラスボスなのでCPU同士で五分よりやや王が有利な53.8%を
採った（実プレイヤーは主人公役のCPUより上手いので、人間相手ならもう少し主人公が
勝つ）。難易度を動かしたい時はこの表で国庫だけ振ればいい。

**⑤ 副業収入は無価値だった。** 0Gで撃てるので試したが、独立seed 41/51/61で
採用版21/60・不採用版21/60と**完全に同数**。周回が2.5周しかないので伸びない。
⚠️ 1つのseedでは+10ptに見えていた。**必ず複数seedで判定すること**。

**⑥ 不動産鑑〇士は死に札ではない（2026-09、前の記述は誤り）。**
`battleCards.js`の⑫クエのデッキに「CPUに詠唱ハンドラが無い死に札」という**古い
コメントが残っており、それを鵜呑みにして一度そう書いてしまった**。実際には
`game.js`に`_cpuMaybeUseAppraiserSpell`（所有地3つ以上＋レベルアップ資金200Gが
残る時）と`_cpuMaybeUseLandSpell`内の`enableAllOwnTileAbilities`分岐（所有地2つ
以上）の**2経路が実装済み**で、`_runCPUTurn`から呼ばれている。実戦ログで1試合中に
2回詠唱するのを確認済み。NPCデッキへ入れてよい。
⚠️ **カードが「CPUに使えるか」はコメントではなく`game.js`の実装で確かめること。**
効果は`_applyPlayerCurse(player,'allTilesAccessTurnsRemaining',2)`＝プレイヤー呪い
なので、**撃つと他のプレイヤー呪いを上書きして消す**（呪いは1枠、後勝ちの不可侵
ルール）点には注意。

⚠️ **seedによる振れが非常に大きい盤面**（同条件でseedごとに15%〜55%まで動く）。
n=20での比較は意味が無い。最低でも独立seed×4＝80戦で判断すること。
