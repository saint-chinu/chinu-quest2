...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v238`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
- **原因はコストカーブ**。川田のモンスター平均コスト86.2G（121G以上が6体＝
  くぐつの剣豪150G×3・混沌の頭150G×2・水神150G・ビッグマーメイド120G）に対し、
  ひなんじょは平均55.5Gで21体中20体が50G以下。**startingCurrency 500G**では
  川田は序盤に展開できず、土地が平均8.6枚 vs 14.0枚と大きく差がつく。
  川田の最終総資産は平均9400〜10200Gで、ゴールの13000Gに届いていない。
...
- ⚠️ **これはCPU対CPUの数字**。実際の⑮は川田がCPU・主人公が人間なので、
  ...
  示す。調整するならコストを下げるか`startingCurrency`を上げる方向。
  未着手（ユーザー判断待ち）。
