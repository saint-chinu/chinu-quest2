...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v240`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
#### 試した調整と効果（いずれも決定打にならず）
効率差(0.80 vs 1.11 pts/G)が支配的で、以下はどれも勝率を動かせていない。
次に効きそうなのは**150G帯（くぐつの剣豪／混沌の頭／水神）を削って
50G帯の優良体へ寄せる**方向だが、⑮のコンセプト（連鎖と自動侵略）に
関わるので未着手。
