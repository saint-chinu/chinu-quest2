...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v275`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
- 実体は`applyCurse(unit, { name:'聖域', traits:['sanctuary','spellUntargetable'] })`。
...
### 単体スペル耐性は「スペルだけ」を弾く
- 聖域が付ける特性は**`spellUntargetable`**で、判定は
  **`_isSpellUntargetableUnit(unit)`**。スペルの対象選択・
  `_applySpellEffect`の最終ガード・CPUの各スペルAI・`_cpuCastSpell`で使う。
- 既存の**`singleTargetImmune`**（カード固有の特性）とは別物。あちらは
  スペルも土地コマンドの特殊効果もまとめて弾く強い耐性で、
  **火炎瓶男・センチネルの能力側の判定は`singleTargetImmune`のまま**にしてある。
  ユーザー指定が「スペルの対象にならない」なので能力までは止めない。
  ⚠️ 聖域の土地は侵略もできないため、能力まで止めると全体スペル以外に
  干渉手段が無くなってしまう。ここを`_isSpellUntargetableUnit`へ広げる時は
  その副作用を承知のうえで。
...
- 回帰テスト: `npm run test:cards`「聖域はモンスターへの呪いとして付く」
  ...
  全体スペルは通る」。旧実装（土地フラグ）へ戻すと4件とも落ちることを確認済み。
