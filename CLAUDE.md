...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v291`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
固定40枚（モンスター20／アイテム8／スペル12）、EXは資本主義の権化2・国士無双2・
**言論封殺2**（チヌ専用、下記）の6枚。
ユーザー指定は「**両者500Gの対等スタート**」「お札ありの長期戦」「**合体系（ガシャーン）
だけ禁止、単にEXなだけなら可**」「属性は自由」。
- **雷の30〜50G先制トリオ（エレキ輝4・サンダーバード4・テンホウ4）が土台**。94マスを
  数で取り、fixer型のお札運用で雷お札を買い集め、放電2と土地レベルアップで雷の相場を
  自分で吊り上げる。くぐつの剣豪3の自動侵略（⑮川田で実証済み）で盤面を広げ続け、
  伸びた雷連鎖に国士無双！！（連鎖数×4）を乗せる。
