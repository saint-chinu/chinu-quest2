...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v218`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
- **`fusagikonda`デッキ構成（2026-08時点、40枚）**: モンスター18
  ...
  ナンカのお守り1）、スペル18（灰塵2／パンデミック4／ホライズン2／
  持たざる者1／毒霧2／遅延行為1／1のダイス1／サイコキネシス1／
  バックファイア4）。dryad/未知の侵略者/狂戦士は全廃し、戦闘列車・供物車両・
  ボムボックリ・サイコキネシスへ入れ替えた。鉄男・アイキャンフライ・
  千本桜は0枚に削り、代わりにパンデミック・ボムボックリを最大枚数(4)へ、
  バックファイア(reverseNextDice、CPU判断は既存の
  `_cpuMaybeUseReverseDiceSpell`をそのまま流用、追加実装なし)を新規に
  最大枚数採用（真剣白刃取り3→1・遅延行為2→1・1のダイス2→1・
  持たざる者2→1で枠を確保）。
