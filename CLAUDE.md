...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v243`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
- ⚠️ **⑮では川田自身を強く巻き込むため勝率が約7pt下がる**（28.3/29.2% →
  ...
  **くぐつの剣豪×3・混沌の頭・くねくね**は常に該当して自爆する。
  ...
  外すなら`battleCards.js`のkawadaから1枚抜いて混沌の頭を2枚へ戻す
  （＝実験Cの28.3/29.2%に戻る）。ユーザー判断待ち。
- 15へ下げた後も⑮は21.7/25.0%で、実験C（28.3/29.2%）には届かない。
