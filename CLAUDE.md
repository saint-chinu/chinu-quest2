...
属性は**連続3マス1セット・4属性均等**（2026-09、ユーザー指定）。
そのまま`board.js`の`*_ROWS`へ貼れる形:
...
**属性配分（検証済み）**: 火/水/雷/森が**それぞれ3マス連続×2セット＝6マス**、
合計24マス。残り**11マスが無属性**。
- ⚠️ **3マス1セットで4属性均等にすると、色付きは24マスが上限**。土地は35マスで、
  3セット×4属性＝12セット＝36マスは1マス足りない。よって2セット×4属性＝
  8セット＝24マスが最大で、余り11マスが無属性になる（算数で決まる）。
- **同じ属性のセット同士は接していない**（各属性がちょうど「サイズ3の連結成分×2」
  であることをスクリプトで確認済み）。つまり1セット取れば連鎖3が確定するが、
  同属性を繋げて連鎖6にはできない。
- セットの位置は盤面全体へ散らしてある:
  火＝`(0,1)-(0,3)`北西の縦通路 と `(5,7)-(6,8)`南東／
  水＝`(8,3)-(9,2)-(9,3)`CP①へ続く北東の腕 と `(3,9)-(5,9)`南の横通路／
  雷＝`(1,4)-(3,4)`中央横通路の西半分 と `(6,5)-(7,6)`東／
  森＝`(4,4)-(6,4)`中央横通路の東半分 と `(0,5)-(0,7)`西の縦通路。
  同属性の2セットが必ず盤面の反対側同士になるので、両方押さえるには
  洞窟を大きく回る必要がある。
