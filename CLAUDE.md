...
`L`の位置がそのまま土地マス、`.`がマス無し。⚠️ **`L`は仮置き**で、
実装時にF/W/T/M/Nのどれかへ置き換える（属性配分は下記のとおり未定）。
...
### まだ未定
- **盤面の属性配分**。ユーザー指定は形とCPとお札の有無だけで、各マスの
  属性は未指定。⚠️ ここは決め方で性格が大きく変わる:
  - **味方の川田(水15)と敵のムール(水19)がどちらも水軸**（ダンボール男は
    無11/火6/雷6）。水マスを多く置くと両陣営に等しく効くので、
    盤面の属性がそのまま有利不利を決めてしまう。
  - 既存のお札マップは2通りの前例がある。⑫海上金融街は火/水/雷/森を
    4方向へ分離配置、⑬豪華客船は**全マス無属性**でプレイヤーが塗る方式。
    ⑬型（全N）なら「掘り出したばかりの岩穴」という舞台にも合い、
    お札の相場も「どちらが先に塗るか」で動く勝負になる。
