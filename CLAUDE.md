...
- カードは全て実装済み: `backfire`(S50G `reverseNextDice` target=anyPlayer)、
  `fireball`(N40G 15ダメージ)、`senbonZakura`(R100G 30ダメージ)、
  `iCanFly`、`homingInstinct`。CHARACTER_DECKSに枚数上限は無いので4枚/3枚可。
...
- 受付の「暗・少女A」は既存の`darkShoujoA`系の立ち絵が流用できそう
  （CHARACTER_DECKSに`shoujoA`/`darkShoujoA`が存在）。
- 周回数は`player.lapsCompleted`があり、`_grantGoalBonus`がスタート通過で走る。
...
2. **強制敗北の仕組み**: 現状**存在しない**。「主人公は勝てない」をどう表現するか
   （目標到達を握り潰す／敗北時も進行する、等）は設計判断が要る。⑮で使った
   `_checkGoalAchievement`のチュートリアル用抑止が参考になる。
3. **全員の初期手札にバックファイア1枚**: 初期手札の指定は
   `_prepareTutorialOpeningHand`（tutorialMode限定）しか無い。非チュートリアルでも
   使えるように一般化が要る。
4. **「半周ごとに交代でバックファイア」のAI**: 一直線での半周（端に到達）の検出と、
   3体が順番に撃つ交代管理を新規に書く必要がある。既存の
   `_cpuMaybeUseDisruptionSpell`等は「撃てる時に撃つ」なので交代制は入っていない。
5. **開始時の大きな2行バナー**: `onGoalAchieved`→`promptGoalAchieved`のような
   全画面演出の前例はあるので、それに倣って新設する。
6. 魚群の王チヌ本人のキャラ定義・立ち絵・専用デッキ（**チヌは盤面に出ない**
   ＝下僕3体だけが戦う、という読みで良いか要確認）。

### 未確定・要確認
- **チヌ自身は盤面に参加するのか**。台詞では「ワイの手を煩わせるほどでない」と
  言って下僕3体を出しているので、1vs3の3体は**ウサギン・ムール・ヒトデマソ**と
  解釈した（チヌは観戦）。ここは着手前に確認すること。
- 負けた後の展開（**地下労働施設に閉じ込められる**）は既定路線だが、
  そのステージ自体もまだ未設計。
