...
## 新ストーリー「川田」(⑮、story.js実装済み・専用マップ未実装)
⑭「王都の番人？？」の次、王都に入った直後のステージ⑮`key:'kawada'`。
**ステージ本体（intro/outro会話・opponents・デッキ・新アイテム）は実装済み**。
**⚠️専用マップが無いので`wip: true`でロック中**（下記「残っている不足分」）。
...
- ⚠️ **board.jsにid:'kawada'の専用マップが無い**。全ステージはmapId省略時
  `stage.key`をそのままmapIdに使う（main.js`startStoryBattle`）ため、この
  ままだと`getMap()`がMAPS[0]（①ヒトデの縄張り）へ静かにフォールバックし、
  盤面・背景が食い違う事故になる。**マップ（`ROWS`定義・背景画像・
  checkpointBonus等）を追加し、story.js`kawada`の`wip: true`を外すまでが
  セット**（回帰テスト`tests/newCards.test.mjs`「ステージ15(川田)は専用
  マップが無い間、必ずwipでロックされている」がこの対応関係を強制する）。
