...
- Service Worker (`public/sw.js`) の `CACHE_NAME` を**毎デプロイbumpする**
  （現在 `chinuquest2-v298`）。bumpしないと古いJS/CSSがキャッシュから配信される。
...
- **結末のセリフ（ユーザー指定・確定稿）**: 真エンドはタイトル回収の小咄
  （チヌ「このゲームの名前...なぜこの名前かわかるか？」→主人公「急にメタ的な話は
  やめて！！」→「チヌ（と）クエ（が）す（ごい）つ（よい）」→「Chinu Que Sugoku Tsuyoi」→
  「ChinuQueST...」）の後、チヌ「お前がたどり着いた場所は、多数の魚達の屍の上という
  ことをゆめゆめ忘れるな」と共にEXを入手。主人公の「急に」は原文「旧に」を変換ミスと
  見て直してある。バッドエンドは
...
15列×14行・**実マス94**で本編最大（従来最大は⑤の85）。横棒3本＋縦棒で「王」を描く。
- ⚠️ **棒を1マス幅にしないこと**。横棒の端6か所が全部行き止まりになり、往復専用の
  盤面に退化する。横棒2行厚・縦棒3列厚にしてあるので端がUターンの小ループになり、
  **行き止まりは0**（分岐点82）。
- CPは4つ。上の横棒の左端(0,2)・右端(0,12)、**中央の玉座(7,7)**、下の横棒の
  右端(13,14)。スタートは下の横棒の左端(13,0)。`requireAllCheckpoints`と合わせて
  「王の字を端から端までなぞらないとゴールできない」形。
- 属性は火水雷森が各21マス＋無属性5マス、**最大連鎖3**。無属性5マスは縦棒の芯へ
  互いに隣接しないよう点在させてあるので無属性の連鎖は起きない。
