...
- ランダム配置は`random.Random(20260901)`で抽選した1回の結果を採用している
  （再現可能）。振り直すならこのseedを変えること。
- **連鎖の分布に1つだけ偏りが出ている**（乱数の結果、スクリプトで確認済み）:
  - 火: **4** + 3 + 1 … `(0,4)`が既存の火3セット`(0,1)-(0,3)`に隣接して
    左端の縦通路が**4連鎖**になった。**このマップで唯一の4連鎖**。
  - 水: 3 + 3 + 1 + 1／雷: 3 + 3 + 1 + 1／森: 3 + 3 + 1 + 1
  - つまり火だけ連鎖4を作れて他は3止まり。均等にしたいならseedを振り直すか、
    `(0,4)`だけ別属性へ動かす。ユーザー指定は「ランダム」なので現状はこのまま。
- セットの位置は盤面全体へ散らしてある:
  火＝`(0,1)-(0,4)`北西の縦通路 と `(5,7)-(6,8)`南東 と単独`(2,8)`／
