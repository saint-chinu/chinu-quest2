...
## ⑱「決戦」構想（2026-08 ユーザー指定・**未実装／仕様のみ**）
⚠️ **まだ一行もコード化していない。** ⑰でムールとダンボール男を倒した後の決戦。

### 構成（ユーザー指定）
1. **まず チヌ単体と1vs1** で戦う。
2. **主人公が勝つと、敵側に クエ が登場して 2vs1** になる
   （敵＝チヌ＋クエ、主人公は1人）。
...
### 実現可能性（調査済み）
- ✅ **戦闘中に敵を乱入させる機構は既にある**。`_addStoryAssistPlayer(cfg)`
  （game.js 620付近）は**陣営に依存しない汎用実装**で、渡すconfigの
  `allianceId`で味方にも敵にもできる（現状の唯一の味方専用要素は呼び出し側の
  ログ文言「〜が紅組に参戦した！」だけ）。⑯のメモにある`storyAssistEvent`の
  仕組みごと流用できる。
- 必要なのは**発火条件の差し替え**: 現状は「敵の総資産が主人公の`ratio`倍」
  だが、⑱は「**チヌを倒した／主人公が勝利寸前**」で撃つ必要がある。
- 乱入者は「主人公のホームゴール地点」から開始する実装なので、
  敵として出す場合の開始位置が妥当かは要確認。

### まだ未定
- 1vs1と2vs1で**盤面を分けるのか、1つの盤面で連続させるのか**
  （`_addStoryAssistPlayer`を使うなら後者＝1盤面で継続が自然）。
- 目標総資産・盤面・チヌとクエの専用デッキ。
- **川田はここに参加しないのか**（⑰では味方だったが、⑱は2vs1＝主人公1人）。
- 決戦の会話（intro/outro）。クエが寝返る／雇われる動機。
