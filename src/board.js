...
// 15列×14行、実マス46。横棒3本（上=cols2-12 / 中=cols3-11 /
// 下=cols0-14）と縦棒（col7）をそれぞれ1マス幅で描く、細い「王」型。
// 旧盤面のような厚い棒を廃止し、同じ横棒のマスが上下に重ならないようにした。
// これにより、移動時に隣接マスが大量に候補へ出る状態を避けつつ、王の字の
// 3本の横棒と中央の縦棒をたどれる。
//
// 成立確認済み（4方向隣接で実際に辿って検証、tests/newCards.test.mjs）:
// ・全46マスがスタートから到達可能。横棒の両端を含む行き止まりは6つ。
// ・CPは4つ。上の横棒の左端(0,2)・右端(0,12)、中央の玉座(6,7)、下の横棒の
//   右端(13,14)。スタートは下の横棒の左端(13,0)。
// ・CPの番号順（gz→gxの走査順）は ①(0,2) → ②(0,12) → ③玉座(6,7) → ④(13,14)。
//
// 属性は火水雷森が各10マス＋無属性1マス、最大連鎖3。無属性は縦棒の芯(2,7)に
// 1マスだけ置いて、無属性の連鎖が発生しないようにしている。
